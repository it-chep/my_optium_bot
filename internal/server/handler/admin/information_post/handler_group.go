...
import (
	...
	"github.com/it-chep/my_optium_bot.git/internal/server/handler/admin/information_post/delete_post"
	"github.com/it-chep/my_optium_bot.git/internal/server/handler/admin/information_post/delete_theme"
	...
	"github.com/it-chep/my_optium_bot.git/internal/server/handler/admin/information_post/update_post"
	"github.com/it-chep/my_optium_bot.git/internal/server/handler/admin/information_post/update_theme"
)

type HandlerGroup struct {
	GetPostsThemes  *get_posts_themes.Handler
	CreatePostTheme *create_post_theme.Handler
	DeleteTheme     *delete_theme.Handler
	UpdateTheme     *update_theme.Handler
	...
	DeletePost            *delete_post.Handler
	UpdatePost            *update_post.Handler
}
...
func NewGroup(adminModule *admin.Module) *HandlerGroup {
	return &HandlerGroup{
		...
		DeleteTheme:     delete_theme.NewHandler(adminModule),
		UpdateTheme:     update_theme.NewHandler(adminModule),
		...
		DeletePost:            delete_post.NewHandler(adminModule),
		UpdatePost:            update_post.NewHandler(adminModule),
	}
}
