...
// GetNextPostByTheme возвращает пост по теме
func (r *Repository) GetNextPostByTheme(ctx context.Context, patientTgID int64, postTheme information.ThemeID) (_ information.Post, err error) {
	sql := `
		select ip.* from information_posts ip
				 join posts_themes ps on ip.posts_theme_id = ps.id
				 join patient_posts pp on ip.id = pp.post_id
	...
	var nextPost dao.InformationPostsDao
	err = pgxscan.Get(ctx, r.pool, &nextPost, sql, args...)

	return nextPost.ToDomain(), nil
}

// GetAdditionalPost получение дополнительный постов
func (r *Repository) GetAdditionalPost(ctx context.Context, patientTgID int64) (_ information.Post, err error) {
	sql := `
		select ip.* 
		from information_posts ip
		    join posts_themes ps on ip.posts_theme_id = ps.id
	...
	args := []interface{}{patientTgID}
	var nextPost dao.InformationPostsDao
	err = pgxscan.Get(ctx, r.pool, &nextPost, sql, args...)
	return nextPost.ToDomain(), err
}
...
func (r *Repository) GetRepetitionsCount(ctx context.Context, patientTgID, scenarioID int64) (count int64, err error) {
	...
	err = pgxscan.Get(ctx, r.pool, &count, sql, args...)
	return count, err

}

func (r *Repository) UpdateRepetitionsCount(ctx context.Context, patientTgID, scenarioID, repetitionsCount int64) (err error) {
	sql := `
		INSERT INTO repetitions (patient_tg_id, scenario_id, repetitions_count) 
		VALUES ($1, $2, $3)
		ON CONFLICT (patient_tg_id, scenario_id) 
		DO UPDATE SET repetitions_count = $3	
	`
