...
(
    ...
    name        varchar(255), -- Название темы (мотивация, обязательная тема)
    is_required bool          -- флаг обязательная ли тема
);
...
(
    ...
    name            varchar(255), -- название поста для отображения в админке
    posts_theme_id  bigint,       -- id темы, к которой относится пост
    order_in_theme  int,          -- порядковый номер поста в теме
...
(
    ...
    patient_id  bigint,          -- id пациента
    post_id     bigint,          -- id поста
    is_received bool,            -- флаг получил ли пользователь данный пост
